...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        else
        {
            ...
            //which child of old root will be the new root? --> is it arbitrary?
    ...
    // function returns false).
    {
        ...
        {   // Target was found and there is no left subtree, so we can
            ...
            // STUDENT WORK
            oldroot_ptr=root_ptr; //this is diff
            root_ptr=root_ptr->right(); //make right child the new root
            delete oldroot_ptr;
            return true;
    ...
    // of the target were removed.
    {
        ...
        if (target < root_ptr->data( )) //remove left node?
        {   // Continue looking in the left subtree
        ...
        if (target > root_ptr->data( )) //remove right node?
        {   // Continue looking in the right subtree
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
                // STUDENT WORK
                if(cursor->left()==NULL){
                    binary_tree_node<Item> *temp = new binary_tree_node<Item>(entry, NULL, NULL); //create a new node with the entry
                    cursor->set_left(temp); //set cursor's left to new node ^
                    done=true;
                }
                else{
                    cursor=cursor->left();
...
//                cursor.set_left(temp);
            ...
            {   // Go right
                ...
                if(cursor->right()==NULL){
                    binary_tree_node<Item> *temp = new binary_tree_node<Item>(entry, NULL, NULL);
    ...
    template <class Item>
    void bag<Item>::operator +=(const bag<Item>& addend) //this=this+addend
    {
    ...
    // method.
    {
        ...
        {
            // STUDENT WORK
            insert(addroot_ptr->data());//will insert all the left and right items recursively
