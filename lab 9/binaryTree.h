...
namespace coen79_lab9
{
    ...
    // Library facilities used: cstdlib
    {
        ...
        else
        {
            ...
            return new binary_tree_node<Item>( root_ptr->data(), l_ptr, r_ptr); //construct new tree with same data and links as this
        }
    }
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr == NULL)
            ...
            // STUDENT WORK
            return 1 + tree_size(node_ptr->left()) + tree_size(node_ptr->right()); //recursively find the size of each of the branches
    }
}
